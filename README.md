<p align="center">
  <img 
    src="https://readme-typing-svg.demolab.com?font=Fira+Code&size=22&pause=1200&color=38BDF8&center=true&vCenter=true&width=750&lines=Cloud-Native+Systems+Engineer;OpenTelemetry+%7C+Observability;Kubernetes+%7C+OSS+Contributor;Build+Small.+Build+Correct." 
    alt="Typing SVG" 
  />
</p>

---

## Hi, I'm Tushar Yadav 👋

Aspiring software engineer focused on **cloud-native systems, observability, and open-source infrastructure**.  
Head of R&D at Pixel Coding Club, working on real-world systems that prioritize correctness, scalability, and safety.

---

## 🔧 Current Technical Focus

- OpenTelemetry & observability pipelines
- Kubernetes & cloud-native systems
- Backend & systems programming
- Secure, reproducible infrastructure

---

## 🧑‍💻 Languages & Tools (Actively Used)

### Primary (OSS / Infra)
- Go
- Python
- TypeScript / JavaScript

### Also worked with
- Java, Kotlin, C/C++
- SQL
- Docker, Kubernetes
- Linux (Ubuntu, RHEL, Fedora)

---

## 📊 GitHub Language Stats (Auto-updating)

> Generated directly from my public repositories (no hard-coded data).

<p align="center">
  <img 
    src="https://github-readme-stats.vercel.app/api/top-langs/?username=codingnoobno1&layout=compact&theme=tokyonight&hide=html,css,scss,shell,batchfile&langs_count=8" 
    alt="Top Languages"
  />
</p>

---

## ⚙️ GitHub Activity (Real-Time)

<p align="center">
<img src="https://github-readme-stats.vercel.app/api/top-langs/?username=codingnoobno1&layout=compact&theme=tokyonight&hide=html,css,scss,shell,batchfile&langs_count=8&cache_seconds=86400" />

</p>

---

## 🧠 Open Source & Learning Philosophy

- Prefer **small, correct contributions** over large unsafe changes
- Follow upstream design principles
- Value observability and clarity over cleverness
- Learn by reading code, not just documentation

---

## 🚀 What I'm Working On

- Contributing to OpenTelemetry Collector ecosystem
- Exploring adaptive observability patterns (read-only, insight-driven)
- Improving developer experience in cloud-native tooling

---

## 📫 Connect

- LinkedIn: https://www.linkedin.com/in/tushar-yadav-a112b1148/
- GitHub: https://github.com/codingnoobno1

> *Strong systems are built by engineers who respect constraints.*

